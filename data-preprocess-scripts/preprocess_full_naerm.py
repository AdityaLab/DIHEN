...
def fuzzyfinder(user_input, collection):
    ...
    for item in collection:
        ...
        if match:
            if user_input=='Distribution_Substations':
...
def get_domain_constrain_pj(node,par,trans_volt_map,es_volt_map,G,nodes_map,maxVolt=765):
    ...
    if par_nm.split(':')[0]=='Transmission_Substations':
        if node_nm.split(':')[0]=='Distribution_Substations'\
            or node_nm.split(':')[0]=='Transmission_Substations':
    ...
    elif par_nm.split(':')[0]=='Distribution_Substations':
...
def read_matt_graph(filedir):
    ...
    if not os.path.exists(path): 
        os.makedirs(path)
    nodes_list=open(filedir+'naerm-index.txt','r')
    nodes_file=open(path+'all_nodes_index.txt','w')
    nodes_map={}
    num_trans=0
    transmission_nodes=open(path+'transmission_nodes.txt','w') #arrays
    G=nx.read_edgelist(filedir+"naerm-graph.txt", delimiter='\t', create_using=nx.DiGraph(),nodetype=int) 
    for line in nodes_list:
        ...
        if node_id not in nodes_map.keys():
            ...
            if node_name.split(':')[0]=='Transmission_Substations':
...
def uncertainty_with_domain(filedir,edgefile):
    ...
    path=filedir+'/preprocessed_data/'
    #infile='data/v9/urbannet-api-graph/'
    infile=filedir+'edge-files/'
    trans_info_data=pd.read_csv(infile+'_Transmission_Substations.node',delimiter=',',index_col=False,low_memory=False)
    ...
    es_info_data=pd.read_csv(infile+'_Distribution_Substations.node',delimiter=',',index_col=False,low_memory=False)
    es_id=es_info_data['NODE_ID'].values
    #es_volt=es_info_data['NOMINAL_KV'].values
    es_volt=es_info_data['NOMKVMAX'].values
...
if __name__=='__main__':
    filedir='../data/naerm/' #sys.argv[1]
    edge_file='naerm_edges_rule_based_pj_p_v2' #added substation volts
