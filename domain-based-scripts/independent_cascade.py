...
def independent_cascade(G,seeds,filename,pre_computed_p=False,alpha=0.1,steps=0):
    ...
    if steps <= 0:
        ...
        return _diffuse_all(DG, A,pre_computed_p) #input for dynamic model:nodes_pj,num_failed_parents,alpha
    # perform diffusion for at most "steps" rounds
    return _diffuse_k_rounds(DG,A,steps,pre_computed_p)

def _diffuse_all(G,A,pre_computed_p):
  #print("diffuse all")
  tried_edges = set()
  layer_i_nodes = [ ]
  ...
  while True:
    ...
    (A, activated_nodes_of_this_round, cur_tried_edges) = \
        _diffuse_one_round(G,A,tried_edges,pre_computed_p)
...
def _diffuse_k_rounds(G,A,steps,pre_computed_p):
  tried_edges = set()
  layer_i_nodes = [ ]
...
def _diffuse_one_round(G,A,tried_edges,pre_computed_p):
  ...
  for s in A:
    for nb in G.neighbors(s):
      if nb in A or (s, nb) in tried_edges or (s, nb) in cur_tried_edges:
        continue
      #extra input for dynamic system: nodes_prob[nb],num_failed_parents[nb],alpha
      if _prop_success(G, s, nb,pre_computed_p):
...
def _prop_success(G, src, dest,pre_computed_p):
  #print("5")
  #pij=G[src][dest]['act_prob']*(1-np.power(alpha,n-1)*pj)
  pij=G[src][dest]['act_prob']
  if pij > 1 or pij<0:
      raise Exception("edge activation probability:", \
        G[src][dest]['act_prob'],\
            "cannot be larger than 1 or negative")
  if pre_computed_p:
      return G[src][dest]['p']<pij
