...
def check_model_robustness(filedir,outdir,outfile,edgefile,reg,K,p,m,critical_fac,cases):
    infile='data/v9/'
    ...
    for case in cases:
            ...
            if case==1:#check effect of P 
                tmp_data['act_prob']=b1*df.kij*(1-df.pj)
            elif case==2: #check effect of U
                tmp_data['act_prob']=1-b1*df.pj
            elif case==3: #check effect of K
                tmp_data['act_prob']=b1*df.kij
            elif case==4: #check effect of U for domain rule
                tmp_data['act_prob']=get_act_prob(df,nodes_map,b1,True)
            elif case==5: #check effect of U for no domain rule
                tmp_data['act_prob']=get_act_prob(df,nodes_map,b1,False)
            elif case==6: #check effect random edge weights with sampled from mean and var P
...
def check_network_scenario(filedir,outdir,outfile,edgefile,k,p,m,critical_fac,reg):
    ...
    df = pd.read_csv(filedir+edgefile+'.txt',delimiter=',',names=['u','v','kij','pj','p'])
    df['act_prob']=df.kij*(1-df.pj)
...
if __name__=='__main__':
    ...
    #outdir='output/domain_added_tl_tes/scenario3/'#sys.argv[2]
    outdir='output/baseline_results_all_data/'#sys.argv[2]
    #outfile='toy_ddic' #sys.argv[4] 
    #outfile='power_system_graph' #sys.argv[3]   
    outfile='ablation_TX' #sys.argv[3] 
    ...
    region='TX'
    #beta2=[1]
    #beta1=[1,0.09,0.07,0.05,0.03,0.01]
    #beta2=[1,0.09,0.07,0.05,0.03,0.01]
    cases=[1,2,3,4,5,6]
    #1: beta1 for P 2: beta1 for U
    #3: beta1 for K #4: beta1 for U in domain-rule 
    #5 beta1 for U not in domain rule
    check_model_robustness(filedir,outdir,outfile,edgefile,region,K,p,m,critical_fac,cases)
    
    #region='TX'
    #check_network_scenario(filedir,outdir,outfile,edgefile,k,p,m,critical_fac,region)
